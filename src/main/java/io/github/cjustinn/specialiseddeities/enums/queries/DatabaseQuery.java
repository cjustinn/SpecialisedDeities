...
public enum DatabaseQuery {
    ...
    // Deity Altars
    SelectAllDeityAltars(
            "SELECT world, x, y, z, deity, creator, created FROM sd_deity_altars;",
            "SELECT world, x, y, z, deity, creator, created FROM sd_deity_altars;"
    ),
    // Insert Queries
    // Users
    ...
    // Collective Points Transactions
    ...
    // Deity Altars
    InsertDeityAltar(
            "INSERT INTO sd_deity_altars (world, x, y, z, deity, creator) VALUES (?, ?, ?, ?, ?, ?);",
            "INSERT INTO sd_deity_altars (world, x, y, z, deity, creator) VALUES (?, ?, ?, ?, ?, ?);"
    ),
    // Update Queries
    // Users
    ...
    DeleteAltar(
            "DELETE FROM sd_deity_altars WHERE world = ? AND x = ? AND y = ? AND z = ?;",
            "DELETE FROM sd_deity_altars WHERE world = ? AND x = ? AND y = ? AND z = ?;"
    );
